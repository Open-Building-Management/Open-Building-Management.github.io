...
function createCircuitUrl(root){
  ...
  */
  ...
  if (energy) circuiturl += "&energy="+energy;
  if (tint) circuiturl += "&tint="+tint;
  return(circuiturl);
}
  ...
  $.ajax({
    ...
    success(data) {
      if (data["Tint"]) {
        ...
        // ajout d'une légende
        // carré bleu clair > occupation
        // pointillé rouge > énergie
        let occColor = "#90cef4";
  ...
  $.ajax({
    ...
    success(data) {
      if (data["stats"]){
        ...
        // échelle des y - on n'affiche rien sauf pour l'énergie
        ...
        if (data["costs"]) {
          ...
          let xx = margin.right / 2;
          let yy = margin.top + 2*hauteur / 3;
          addVerticalLabel(costs,"% temps à puissance max.", xx, yy, 10);
          ...
          //échelle des y de 0 à 100 - pas de 10
          let y_axis = d3.axisRight().scale(y);
          costs.append("g")
            .attr("transform", "translate(" + 0.65*margin.left + ", 0)")
            .style("font", "7px arial")
            .call(y_axis);

          // 5 lignes horizontales pour servir de repères - pas de 20
          ...
          costs.append("g")
             ...
             .call(make_y_gridlines()
               .tickSize(-largeur + margin.right + margin.left)
               .tickFormat("")
             )
        ...
        if (click === true) {
          ...
          // ajout d'une interactivité sur les rectangles de consommation énergétique
          // on ajoute un lien permettant l'affichage des graphiques de données brutes
          if (data["costs"]) {
            costs.data(energy).selectAll("g")
              .on("click", function(d) {
                ...
                let energy = $("#energy").val();
                let tint = $("#tint").val();
                if (energy) rawurl += "&energy="+energy;
                if (tint) rawurl += "&tint="+tint;
                raw(rawurl);
              });
          }
          // idem sur les rectangles des histogrammes de confort intérieur
